...
from validate_workspace_promotion import (
    invariant_failures,
    reference_transform,
)


class GlobalWorkspacePromotionTests(unittest.TestCase):
    def test_frozen_fixture_hashes_and_holdout_are_disjoint(self) -> None:
        manifest = json.loads(
            (ROOT / "promotions/cognition.global_workspace.v1.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_reference_fixtures_preserve_workspace_invariants(self) -> None:
        fixture = ROOT / "validation/equivalence/global_workspace_v1.jsonl"
